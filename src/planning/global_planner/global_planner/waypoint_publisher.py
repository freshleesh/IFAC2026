...
from global_planner.track_bounds import TrackBounds
...
class WaypointPublisher(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('waypoint_file', 'global_waypoints.csv')
        ...
        self.sp_wp_marker_pub = self.create_publisher(
            MarkerArray, '/sp_waypoints/markers', latched_qos)
    ...
    def _load_global_waypoints(self):
        """Load waypoint_file CSV → publish markers + WpntArray on /global_waypoints."""
        data = self._load_csv(self.waypoint_file)
        if data is None:
            return
    ...
    def _load_sp_waypoints(self):
        """Load shortest_path.csv → publish markers + WpntArray."""
        data = self._load_csv('shortest_path.csv')
        if data is None:
            return
        ...
        # Markers (green line strip)
        self._sp_wp_marker = self._create_line_strip_markers(
            x, y, ns='sp_waypoints', r=0.0, g=1.0, b=0.0, width=0.03)
        self.sp_wp_marker_pub.publish(self._sp_wp_marker)

        # WpntArray
        self._sp_wp_wpnt = self._csv_to_wpnt_array(data)
        self.sp_wp_wpnt_pub.publish(self._sp_wp_wpnt)
