...
class WallFollowNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f'target_dist: {self.target_dist}, kp: {self.kp}, ki: {self.ki}, kd: {self.kd}, speed: {self.speed}, lookahead: {self.lookahead}')
    ...
    def _compute(self, dt):
        ...
        # find right wall points from LiDAR scan
        right_wall_points = []
        ...
        if len(right_wall_points) < 1:
            self.get_logger().warn('Not enough right wall points detected, skipping control')
            return 0.0, self.speed  # go straight if we can't detect the wall


        # find numerical properties of the right wall points using linear regression
        x = right_wall_points[:, 0]
        y = right_wall_points[:, 1]
        A = np.vstack([x, np.ones(len(x))]).T
        m, c = np.linalg.lstsq(A, y, rcond=None)[0]  # line: y = mx + c
        alpha = math.atan(m)  # wall angle
        wall_dist = abs(c) / math.sqrt(m**2 + 1)  # perpendicular distance to wall


        # project the wall distance error forward using lookahead
        projected_dist = wall_dist - self.lookahead * math.sin(alpha)
        error = self.target_dist - projected_dist
        ...
        speed = self.speed  # constant speed for wall following

        self.get_logger().info(f'Wall angle (alpha): {math.degrees(alpha):.2f} deg, Wall distance: {wall_dist:.2f} m, Steering: {steer:.2f} rad, Error: {error:.2f} m')
