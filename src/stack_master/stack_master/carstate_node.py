...
class CarStateNode():
    def __init__(self, prop_state=False):
        ...
        self.IMU_TOPIC = self._get_param_or_default('/carstate_node/imu_topic', default='/imu/data')
    ...
    def state_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.last_pose is not None and carstate_pose_msg.header.stamp.to_sec() - self.last_pose.header.stamp.to_sec() > 0.0:
                ...
                pass
                # self.get_logger().warning("[Car State] No last pose to differentiate with")
