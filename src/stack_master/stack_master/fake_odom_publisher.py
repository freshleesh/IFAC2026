...
class FakeOdomPublisher:
    def __init__(self):
        ...
        map_name = self._get_param_or_default("~map", "gazebo_wall_3d_rc_car_10th_timeoptimal")
        speed_scale = self._get_param_or_default("~speed_scale", 1.0)
        rate_hz = self._get_param_or_default("~rate", 50.0)
        ...
        self.get_logger().info("Loaded %d waypoints from %s" % (self.n_wpnts, map_name))

        self.pub = self.create_publisher(Odometry, "/glim_ros/base_odom", 10)
