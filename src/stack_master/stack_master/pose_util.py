...
def listener():
    ...
    in_topic  = self._get_param_or_default("~in_topic", "car_state/odom")
    out_topic = self._get_param_or_default("~out_topic", "car_state/pose")
    
    self.create_subscription(PoseStamped, in_topic, callback, 10)

    global pub
    pub = self.create_publisher(PoseWithCovarianceStamped, out_topic, 10)
