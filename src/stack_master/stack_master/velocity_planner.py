...
class VelocityPlanner:
    ...
    def __init__(self):
        ...
        self.a_y_max = self._get_param_or_default("/vel_planner/a_y_max", 5.0) # friction coefficient
        self.a_x_max = self._get_param_or_default("/vel_planner/a_x_max", 5.0) # friction coefficient



        self.v_max = self._get_param_or_default("/vel_planner/v_max", 10.0)
        self.drag_coeff = self._get_param_or_default("/vel_planner/drag_coeff", 0.0136)
        self.m_veh = self._get_param_or_default("/vel_planner/m_veh", 3.5)
        self.dyn_model_exp = self._get_param_or_default("/vel_planner/dyn_model_exp", 1.3)


        ggv_path = os.path.join(get_package_share_directory('stack_master'), 'config', 'gb_optimizer', "veh_dyn_info", "ggv.csv")
        ax_max_path = os.path.join(get_package_share_directory('stack_master'), 'config', 'gb_optimizer', "veh_dyn_info", "ax_max_machines.csv")
        ...
        self.create_subscription(WpntArray, "/local_waypoints", self.wpnts_callback, 10)
        self.create_subscription(Odometry, "/car_state/odom", self.odom_callback, 10)

        self.speed_pub = self.create_publisher(Float64, "vel_planner", 10)
        self.acc_pub = self.create_publisher(Float64, "/vel_planner_acc", 10)
