...
class Opponent_Trajectory:
    def __init__(self):
        ...
        self.create_subscription(Odometry, '/car_state/odom_frenet', self.car_state_frenet_cb, 10) # car frenet coordinates
        self.create_subscription(WpntArray, '/global_waypoints', self.glb_wpnts_cb, 10) # global waypoints
        self.create_subscription(OpponentTrajectory, '/opponent_trajectory', self.opp_traj_cb, 10) # global waypoints
        #subscribe to state machine
        self.create_subscription(String, '/state_machine', self.state_machine_cb, 10)

        # Retrieve ROS parameters
        self.loop_rate = self._get_param_or_default('~loop_rate', 25)
    ...
    def make_opponent_trajectory(self):
        ...
        while not rospy.is_shutdown(): 
            ...
            if index is not None and self.lap_count >= 1:
                time_diff = self.get_clock().now().to_msg().to_sec() - raw_opponent_traj[index][6]
            ...
            if opponent is not None:
                ...
                if index != old_index: #if the index is the same as the old index we dont update the opponent trajectory (this also gets rid of the delta time = 0 problem in simulation)
                    ...
                    if consecutive_points_off_opptraj > 5 and traversed_distance < track_length/2: #if the opponent is off the predicted trajectory for more than 5 points we discard the measurement (variable)
                        ...
                        self.proj_opponent_trajectory_pub.publish(projected_opponent_traj)
                        start_time = self.get_clock().now().to_msg().to_sec()
                        while(self.opp_traj_count == opp_traj_count_old and self.get_clock().now().to_msg().to_sec() - start_time < 3):
