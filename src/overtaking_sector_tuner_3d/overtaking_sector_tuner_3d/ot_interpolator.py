...
class OvertakingInterpolator:
    ...
    def __init__(self, debug_plot: bool = False) -> None:
        ...
        self.yeet_factor = self._get_param_or_default("/ot_map_params/yeet_factor")
        self.spline_len = int(self._get_param_or_default("/ot_map_params/spline_len", 30))

        # SUBSCRIBE
        self.glb_wpnts_name = "/global_waypoints_scaled"
        self.glb_wpnts_sp_name = "/global_waypoints/shortest_path"
        self.create_subscription(Config, "/dyn_sector_tuner/overtake/parameter_updates", self.dyn_param_cb, 10)
        self.create_subscription(Config, "/dyn_sector_tuner/speed/parameter_updates", self.dyn_speed_param_cb, 10)
        ...
        self.create_subscription(WpntArray, self.glb_wpnts_sp_name, self.glb_wpnts_sp_cb, 10)
        
        # self.get_logger().info(f"[{self.node_name}] Waiting for the frenet conversion service...")
        # rospy.wait_for_service("/convert_glob2frenet_service")
        # self.get_logger().info(f"[{self.node_name}] Frenet conversion service found!")
    ...
    def loop(self):
        self.get_logger().info(f"[{self.node_name}] Waiting for global waypoints...")
        self.get_logger().info(f"[{self.node_name}] Waiting for shortest path waypoints...")
        rospy.wait_for_message(self.glb_wpnts_name, WpntArray)        
        self.get_logger().info(f"[{self.node_name}] Global waypoints received!")
        rospy.wait_for_message(self.glb_wpnts_sp_name, WpntArray)        
        self.get_logger().info(f"[{self.node_name}] Shortest path waypoints received!")
