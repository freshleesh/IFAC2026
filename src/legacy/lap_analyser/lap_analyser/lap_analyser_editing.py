...
class LapAnalyser:
    def __init__(self):
        ...
        self.create_subscription(WpntArray, '/global_waypoints', self.waypoints_cb, 10) # TODO maybe add wait for topic/timeout?
        ...
        self.LOC_METHOD = self._get_param_or_default("~loc_algo", default="slam")

        # Start Line init
        self.start_line = 0

        # Reset start line
        self.create_subscription(Float32, 'update_start_line', self.update_start_line_cb, 10)

        self.create_subscription(Odometry, '/car_state/odom_frenet', self.frenet_odom_cb, 10)  # car odom in frenet frame

        self.create_subscription(EmptyMsg, '/lap_analyser/start', self.start_log_cb, 10)

        # New subscriber for x,y coordinate odom (assumed available)
        self.create_subscription(Odometry, '/car_state/odom', self.odom_xy_cb, 10)
        ...
        self.logfile_par = os.path.join(get_package_share_directory('lap_analyser'), 'data')
    ...
    #                 self.get_logger().warning(lap_time_str)
    #                 self.get_logger().warning(avg_err_str)
    #                 self.get_logger().warning(max_err_str)
    ...
    #                 #     self.map_dir = os.path.join(get_package_share_directory('lap_analyser'), 'maps', self.map_name)
    #                 #     os.makedirs(self.map_dir)
    #                 #     self.get_logger().info(f"LapAnalyser: Saving map to {self.map_dir}")
    ...
    def update_start_line_cb(self, msg):
        ...
        if msg.data > 0:
            ...
            self.get_logger().info(f"LapAnalyser: Updated start line to current position s={self.start_line:.2f}")
    ...
    def reset_lap_stats(self):
        ...
        self.get_logger().info("LapAnalyser: Resetting lap count and statistics.")
        self.lap_count = -1
    ...
    def publish_lap_info(self):
        msg = LapData()
        msg.lap_time = (self.get_clock().now().to_msg() - self.lap_start_time).to_sec()
        self.get_logger().info(
