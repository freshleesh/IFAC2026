...
from t2v_node_msgs.msg import T2VCommand
...
def main():
    ...
    # ── ROS 파라미터 ──
    param_port = self._get_param_or_default("~port", "/dev/ttyACM0")
    baud = self._get_param_or_default("~baud", 115200)

    # ── 포트 자동 감지 시도 ──
    detected = find_t2v_port()
    if detected:
        port = detected
        self.get_logger().info("T2V 수신기 자동 감지: %s (VID:%s PID:%s)", port, T2V_VID, T2V_PID)
    else:
        port = param_port
        self.get_logger().warning("T2V 수신기 자동 감지 실패 → 파라미터 포트 사용: %s", port)

    # ── 퍼블리셔 ──
    pub_cmd = self.create_publisher(T2VCommand, "/t2v/command", 10)
    pub_joy = self.create_publisher(Joy, "/joy", 10)
    ...
    while not rospy.is_shutdown():
        try:
            ...
            self.get_logger().info("포트 연결 성공: %s @ %d baud", port, baud)
        except serial.SerialException as e:
            self.get_logger().warning("포트 열기 실패 (%s): %s — 3초 후 재시도", port, e)
            rospy.sleep(3.0)
            continue
        ...
        try:
            ...
            while not rospy.is_shutdown():
                ...
                self.get_logger().info("수신: %s (주소: 0x%02X)", cmd_name, addr)
            ...
            self.get_logger().warning("포트 연결 해제: %s — 3초 후 재연결 시도", e)
            ser.close()
            rospy.sleep(3.0)
    ...
    if "ser" in dir() and ser.is_open:
        ser.close()
    self.get_logger().info("T2V 노드 종료")
