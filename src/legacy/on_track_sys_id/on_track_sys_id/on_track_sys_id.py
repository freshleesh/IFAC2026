...
class On_Track_Sys_Id:
    def __init__(self):
        ...
        # self.rate = 70
        self.rate = self._get_param_or_default('~loop_rate')
        ...
        self.save_LUT_name = self._get_param_or_default('~save_LUT_name')
        self.save_LUT_name = f"{self.save_LUT_name}_pacejka"
        
        self.plot_model = self._get_param_or_default('~plot_model')

        # Get topic names from parameters (with default values if not set)
        odom_topic = self._get_param_or_default('~odom_topic', '/car_state/odom')
        ackermann_topic = self._get_param_or_default('~ackermann_cmd_topic', '/vesc/low_level/ackermann_cmd_mux/output')
