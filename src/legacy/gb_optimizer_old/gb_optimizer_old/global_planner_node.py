...
class GlobalPlanner:
    ...
    def __init__(self):
        ...
        self.rate = self._get_param_or_default('/global_planner/rate')
        self.test_on_car = self._get_param_or_default('/global_planner/test_on_car')
        self.current_key = ''
        
        self.safety_width = self._get_param_or_default('/global_planner/safety_width')
        self.safety_width_sp = self._get_param_or_default('/global_planner/safety_width_sp')
        self.occupancy_grid_threshold = self._get_param_or_default('/global_planner/occupancy_grid_threshold')

        self.show_plots = self._get_param_or_default('/global_planner/show_plots', False)  # show no plots if False
        
        self.create_map = self._get_param_or_default('/global_planner/create_map')
        create_global_path = self._get_param_or_default('/global_planner/create_global_path')
        ...
        self.map_name = self._get_param_or_default('/global_planner/map')
        self.map_dir = self._get_param_or_default('/global_planner/map_dir')
        ...
        self.lap_count = 0
        self.required_laps = self._get_param_or_default('/global_planner/required_laps')
    ...
    def global_plan_loop(self):
        ...
        if self.map_editor and not self.map_editor_mapping:
            ...
            with open(os.path.join(self.map_dir, self.map_name + '.yaml')) as f:
                ...
                self.get_logger().warning("=" * 80)
                self.get_logger().warning("[GB Planner]: WAITING FOR USER INPUT")
                self.get_logger().warning("[GB Planner]: RViz will open automatically.")
                self.get_logger().warning("[GB Planner]: Please click '2D Pose Estimate' and select starting point on the map.")
                self.get_logger().warning("[GB Planner]: Press ENTER in this terminal when done...")
                self.get_logger().warning("=" * 80)
                ...
                if os.path.exists(rviz_config_path):
                    ...
                    self.get_logger().warning("[GB Planner]: RViz config not found, launching default RViz")
                ...
                self.get_logger().info("[GB Planner]: RViz closed. Proceeding with optimization...")
                ...
                if self.rviz_initial_pose is not None:
                    ...
                    self.get_logger().warning(f"[GB Planner]: Using RViz pose as initial_position: {self.initial_position}")
                else:
                    self.initial_position = [0, 0, 0]  # Default: closest to origin
                    self.get_logger().warning(f"[GB Planner]: No RViz pose received, using default initial_position: [0, 0, 0]")
