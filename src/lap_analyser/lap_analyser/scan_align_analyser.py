...
class LaserMapOverlapChecker:
    ...
    def __init__(self):
        ...
        self.downsample_factor = self._get_param_or_default('~downsample_factor', default=54)
        self.threshold = self._get_param_or_default('~threshold', default=0.1)
        self.frequency = self._get_param_or_default('~frequency', default=5)

        # Subscriber for the map and laser scan
        self.map_sub = self.create_subscription(OccupancyGrid, '/map', self.map_callback, 10)
        self.laser_sub = self.create_subscription(LaserScan, '/scan', self.laser_callback, 10)
        self.trackbounds_sub = self.create_subscription(MarkerArray, '/trackbounds/markers', self.trackbounds_callback, 10)
        self.gb_sub = self.create_subscription(WpntArray, "/global_waypoints", self.glb_wpnts_cb, 10)  # from og wpnts

        # Publishers
        self.debug_lidar_pub = self.create_publisher(MarkerArray, '/scanalign/debug_lidar', 1)
        self.alignment_score_pub = self.create_publisher(Float32, '/scanalign/alignment_score', 1)
        self.alignment_textmrk_pub = self.create_publisher(Marker, '/scanalign/alignment_textmrk', 1)
