...
class Detect :
    ...
    def __init__(self) -> None:
        ...
        # --- Publisher ---
        self.breakpoints_markers_pub = self.create_publisher(MarkerArray, '/detect/breakpoints_markers', 5)
        self.boundaries_pub = self.create_publisher(Marker, '/detect/detect_bound', 5)
        self.obstacles_msg_pub = self.create_publisher(ObstacleArray, '/detect/raw_obstacles', 5)
        self.obstacles_marker_pub = self.create_publisher(MarkerArray, '/detect/obstacles_markers_new', 5)
        if self.measuring:
            self.latency_pub = self.create_publisher(Float32, '/detect/latency', 5)
            self.on_track_points_pub = self.create_publisher(PointCloud2, '/detect/on_track_points', 5)
        ...
        # --- Tunable params ---
        self.rate = self._get_param_or_default("/perception/rate_detect")
        self.lambda_angle = self._get_param_or_default("/perception/lambda_deg")*math.pi/180
        self.sigma = self._get_param_or_default("/perception/sigma")
        self.min_size_m = self._get_param_or_default("/perception/min_size_m", 0.2)
        self.min_2_points_dist = self._get_param_or_default("/perception/min_2_points_dist") # prevent zero division
        ...
        self.create_subscription(WpntArray, '/global_waypoints', self.pathCb, 10)
        self.create_subscription(Odometry, '/car_state/odom_frenet', self.carStateCb, 10)
    ...
    def shutdown(self):
        self.get_logger().warning('Opponent Detection is shutdown')
    ...
    def pathCb(self, data):
        ...
        if (self.s_array is None or self.path_needs_update) and self.converter is not None:
            self.get_logger().info('[Opponent Detection]: received global path')
    ...
    def dyn_param_cb(self, params: object):
        ...
        self.boundaries_inflation = self._get_param_or_default('dyn_perception/boundaries_inflation', 0.1)
    ...
    def main (self):
        self.get_logger().info('[Opponent Detection]: Waiting for global wpnts')
