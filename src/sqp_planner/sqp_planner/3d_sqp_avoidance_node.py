...
class SQPAvoidance3DNode:
    ...
    def dyn_param_cb(self, params: Config):
        ...
        self.lookahead = self._get_param_or_default("dynamic_sqp_tuner_node/lookahead_dist", 15)
        self.avoidance_resolution = self._get_param_or_default("dynamic_sqp_tuner_node/avoidance_resolution", 20)
        self.back_to_raceline_before = self._get_param_or_default("dynamic_sqp_tuner_node/back_to_raceline_before", 5)
        self.back_to_raceline_after = self._get_param_or_default("dynamic_sqp_tuner_node/back_to_raceline_after", 5)
        self.avoid_static_obs = self._get_param_or_default("dynamic_sqp_tuner_node/avoid_static_obs", True)
        ...
        if result.success:
            ...
            if self.track_validator is not None:
                ...
                if not valid:
                    ...
                    self.past_avoidance_d = []
                    empty_msg = OTWpntArray(header=rospy.Header(stamp=self.get_clock().now().to_msg(), frame_id="map"))
    ...
    def initialize_converter(self):
        ...
        self.get_logger().info("[3D SQP] initialized FrenetConverter object")
