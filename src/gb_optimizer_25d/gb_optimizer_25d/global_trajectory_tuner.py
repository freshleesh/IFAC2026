...
class GlobalTrajTuner(object):
    ...
    def __init__(self):
        ...
        self.map_path = self._get_param_or_default("~map_dir")
        ...
        if self.map_name:
            ...
            try:
                ...
                self.get_logger().warning(f"{self.map_infos.data}")
            except FileNotFoundError:
                self.get_logger().warning(f"{self.map_path} param not found. Not publishing")
        else:
        ...
        if feedback.event_type == InteractiveMarkerFeedback.MENU_SELECT:
            if feedback.menu_entry_id <= len(self.main_menu_list):
                ...
                if selected_menu == "Anchor1":
                    ...
                    self.get_logger().warning(f"{selected_menu} is {self.anchor1[0]} waypoint")
                elif selected_menu == "Anchor2":
                    position = self.server.get(m_name).pose.position
                    self.anchor2 = [int(m_name), position.x, position.y, position.z]
                    self.get_logger().warning(f"{selected_menu} is {self.anchor2[0]} waypoint")
                    ...
                    self.initialize_interactive_markers(self.wpnts_data)
                    self.get_logger().warning(f"Finish straightening velocity in z-axis!!")
                elif selected_menu == "Pos_Straighten":
                    self.push_back_waypoints()
                    self.wpnts_data[:,3:5] = straighten_2d(self.anchor1, self.anchor2, self.wpnts_data[:,3:5])
                    self.update_yaw()
                    self.initialize_interactive_markers(self.wpnts_data)
                    self.get_logger().warning(f"Finish straightening position in xy-plane!!")             
                    ...
                    self.initialize_interactive_markers(self.wpnts_data)
                    self.get_logger().warning(f"Finish setting velocity {user_input} m/s!!")             
                    ...
                    if self.pop_up_display.user_input is None:
                        self.get_logger().warning(f"Something Wrong INDEX!!")             
                    else:
                    ...
                    self.get_logger().warning(f"Something Wrong!!")             
    ...
    def initialize_interactive_markers(self, waypoints_data):
        ...
        # self.get_logger().info("Adding waypoints to interactive markers...")
