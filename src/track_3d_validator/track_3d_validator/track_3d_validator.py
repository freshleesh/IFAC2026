...
class Track3DValidator:
    ...
    def _precompute_boundary_s(self):
        ...
        Preferred path: when the bound count equals the wpnt count, assign
        wpnt.s_m directly by index. The bounds in global_waypoints.json
        are generated by projecting each wpnt along its d direction, so
        the index matching is exact (verified: error = 0 m).
        Fallback: if the counts do not match, fall back to the get_approx_s
        projection. That can break at hairpins / apex / XY-overlap
        geometry, so log a warning.
        """
        conv = self.converter
        if not conv.has_track_bounds:
            _logger.warning("[Track3DValidator] No track bounds loaded, Stage 2 disabled")
            return
