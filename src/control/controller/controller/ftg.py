...
class FTG:
    # 원본 ROS1 은 class body 에서 rospy.get_param 호출 (load 시점). ROS2 포팅에서는
    # node 인스턴스 없으면 호출 불가 → default 값으로 변경. instance 생성 후
    # __init__ 에서 외부 노드 ref 받아 갱신 가능.
    DEBUG = False
    ...
    def __init__(self, mapping=False, node=None) -> None:
        """
        Initialize the FTG controller.

        Parameters:
            mapping (bool): FTG가 mapping 모드인지.
            node (rclpy.node.Node, 옵셔널): 외부 노드 ref. 주어지면 declare_parameter
                + create_publisher 활성. None 이면 default 값 + publisher 비활성 (smoke).
        """
        self.mapping = mapping
        self._node = node

        self.radians_per_elem = None
        # ROS2 포팅: 외부 노드 ref 가 있으면 파라미터 read, 없으면 default
        if node is not None:
            self.range_offset = node._get_param_or_default('/state_machine/range_offset', 0.0)
            self.track_width = node._get_param_or_default('/state_machine/track_width', 3.5)
        else:
        ...
        # publisher 도 외부 노드 ref 통해 만들기 (FTG 자체는 Node 아님)
        if node is not None:
            self.best_pnt = node.create_publisher(Marker, '/best_points/marker', 10)
            self.scan_pub = node.create_publisher(MarkerArray, '/scan_proc/markers', 10)
            self.best_gap = node.create_publisher(MarkerArray, '/best_gap/markers', 10)
        else:
            self.best_pnt = None
            self.scan_pub = None
            self.best_gap = None
