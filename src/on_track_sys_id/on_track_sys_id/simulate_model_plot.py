...
class LookupGenerator:
  ...
  def run_generator(self):
    ...
    if PLOT_LOOKUP:
      self.get_logger().warning("Lookup Table has been generated. Close the plot window (press Q) to save the lookup table.")
      # self.plot_lookup()
    self.save_lookup()
    ...
    for i, steer in enumerate(steers):
      for j, vel in enumerate(vels):
        for k, acc in enumerate(accels):
          ...
          plt.show()
          # self.get_logger().info(f"acc: {acc}, sol {sol}")
  ...
  #   self.get_logger().info(f"SAVED LOOKUP TABLE TO: {file_path}, AND {lookup_file_path}")
  #   ############## YH ##############
  ...
  def save_lookup(self):
    ...
    # self.get_logger().info(f"SAVED LOOKUP TABLE TO: {file_path}")
